...
func setupServer() (*testEnv, error) {
	tempDir, defFn, spaceErr := test.NewSpace()
	if spaceErr != nil {
		return nil, spaceErr
	}
	...
	propertySvc, propertySvcClose, propertySvcErr := banyandProperty.NewTestService(tempDir, tempDir, omr, lfs)
	if propertySvcErr != nil {
		defFn()
		return nil, propertySvcErr
	}
	server := property.NewServer(propertySvc, omr, nil, pm)
...
func createRegistry(address string) *property.SchemaRegistry {
	...
	registry := property.NewSchemaRegistryClient(dialOptsPrv, 5*time.Second, 100*time.Millisecond)
...
var _ = ginkgo.Describe("Property Schema Repair Mechanism", func() {
	var (
		env1      *testEnv
		env2      *testEnv
		registry1 *property.SchemaRegistry
	...
	ginkgo.It("should repair inconsistent schema across nodes using updated_at", func() {
		...
		gomega.Expect(updateErr).ShouldNot(gomega.HaveOccurred())
		time.Sleep(200 * time.Millisecond)
		multiNodeRegistry := property.NewSchemaRegistryClient(newMockDialOptionsProvider(), 5*time.Second, 100*time.Millisecond)
	...
	ginkgo.It("should detect existing schemas when handler is registered after node connection", func() {
		...
		newRegistry := property.NewSchemaRegistryClient(newMockDialOptionsProvider(), 5*time.Second, 100*time.Millisecond)
		defer func() {
			if newRegistry != nil {
...
var _ = ginkgo.Describe("Property Resource Lifecycle", func() {
	var (
		env      *testEnv
		registry *property.SchemaRegistry
		goods    []gleak.Goroutine
	...
	ginkgo.It("should handle complete stream lifecycle with all related resources", func() {
		ctx := context.Background()
		group := &commonv1.Group{
			Metadata: &commonv1.Metadata{Name: "lifecycle-group"},
			Catalog:  commonv1.Catalog_CATALOG_STREAM,
			ResourceOpts: &commonv1.ResourceOpts{
				ShardNum: 2,
			},
		...
		gomega.Expect(registry.CreateGroup(ctx, group)).ShouldNot(gomega.HaveOccurred())
		stream := &databasev1.Stream{
			Metadata: &commonv1.Metadata{Name: "lifecycle-stream", Group: "lifecycle-group"},
		...
		gomega.Expect(streamErr).ShouldNot(gomega.HaveOccurred())
		indexRule := &databasev1.IndexRule{
			Metadata: &commonv1.Metadata{Name: "lifecycle-index-rule", Group: "lifecycle-group"},
		...
		gomega.Expect(registry.CreateIndexRule(ctx, indexRule)).ShouldNot(gomega.HaveOccurred())
		binding := &databasev1.IndexRuleBinding{
			Metadata: &commonv1.Metadata{Name: "lifecycle-binding", Group: "lifecycle-group"},
			Rules:    []string{"lifecycle-index-rule"},
			Subject: &databasev1.Subject{
				Catalog: commonv1.Catalog_CATALOG_STREAM,
		...
		gomega.Expect(registry.CreateIndexRuleBinding(ctx, binding)).ShouldNot(gomega.HaveOccurred())
		retrievedStream, getStreamErr := registry.GetStream(ctx, &commonv1.Metadata{Name: "lifecycle-stream", Group: "lifecycle-group"})
		gomega.Expect(getStreamErr).ShouldNot(gomega.HaveOccurred())
		gomega.Expect(retrievedStream.TagFamilies).Should(gomega.HaveLen(2))
		rules, listRulesErr := registry.ListIndexRule(ctx, schema.ListOpt{Group: "lifecycle-group"})
		gomega.Expect(listRulesErr).ShouldNot(gomega.HaveOccurred())
		gomega.Expect(rules).Should(gomega.HaveLen(1))
		bindings, listBindingsErr := registry.ListIndexRuleBinding(ctx, schema.ListOpt{Group: "lifecycle-group"})
		gomega.Expect(listBindingsErr).ShouldNot(gomega.HaveOccurred())
		gomega.Expect(bindings).Should(gomega.HaveLen(1))
		_, deleteBindingErr := registry.DeleteIndexRuleBinding(ctx, &commonv1.Metadata{Name: "lifecycle-binding", Group: "lifecycle-group"})
		gomega.Expect(deleteBindingErr).ShouldNot(gomega.HaveOccurred())
		_, deleteRuleErr := registry.DeleteIndexRule(ctx, &commonv1.Metadata{Name: "lifecycle-index-rule", Group: "lifecycle-group"})
		gomega.Expect(deleteRuleErr).ShouldNot(gomega.HaveOccurred())
		_, deleteStreamErr := registry.DeleteStream(ctx, &commonv1.Metadata{Name: "lifecycle-stream", Group: "lifecycle-group"})
		gomega.Expect(deleteStreamErr).ShouldNot(gomega.HaveOccurred())
		_, deleteGroupErr := registry.DeleteGroup(ctx, "lifecycle-group")
		gomega.Expect(deleteGroupErr).ShouldNot(gomega.HaveOccurred())
	})

	ginkgo.It("should handle complete measure lifecycle with topN aggregation", func() {
		ctx := context.Background()
		group := &commonv1.Group{
			Metadata: &commonv1.Metadata{Name: "measure-lifecycle-group"},
			Catalog:  commonv1.Catalog_CATALOG_MEASURE,
			ResourceOpts: &commonv1.ResourceOpts{
				ShardNum: 2,
			},
		...
		gomega.Expect(registry.CreateGroup(ctx, group)).ShouldNot(gomega.HaveOccurred())
		measure := &databasev1.Measure{
			Metadata: &commonv1.Metadata{Name: "lifecycle-measure", Group: "measure-lifecycle-group"},
			Entity:   &databasev1.Entity{TagNames: []string{"service"}},
			TagFamilies: []*databasev1.TagFamilySpec{
				{
					Name: "default",
		...
		gomega.Expect(measureErr).ShouldNot(gomega.HaveOccurred())
		topN := &databasev1.TopNAggregation{
			Metadata: &commonv1.Metadata{Name: "lifecycle-topn", Group: "measure-lifecycle-group"},
			SourceMeasure: &commonv1.Metadata{
				Name:  "lifecycle-measure",
				Group: "measure-lifecycle-group",
			},
			FieldName:       "latency",
			FieldValueSort:  modelv1.Sort_SORT_DESC,
			CountersNumber:  100,
		...
		gomega.Expect(registry.CreateTopNAggregation(ctx, topN)).ShouldNot(gomega.HaveOccurred())
		topNAggs, topNListErr := registry.TopNAggregations(ctx, &commonv1.Metadata{Name: "lifecycle-measure", Group: "measure-lifecycle-group"})
		gomega.Expect(topNListErr).ShouldNot(gomega.HaveOccurred())
		gomega.Expect(topNAggs).Should(gomega.HaveLen(1))
		gomega.Expect(topNAggs[0].Metadata.Name).Should(gomega.Equal("lifecycle-topn"))
		_, deleteTopNErr := registry.DeleteTopNAggregation(ctx, &commonv1.Metadata{Name: "lifecycle-topn", Group: "measure-lifecycle-group"})
		gomega.Expect(deleteTopNErr).ShouldNot(gomega.HaveOccurred())
		_, deleteMeasureErr := registry.DeleteMeasure(ctx, &commonv1.Metadata{Name: "lifecycle-measure", Group: "measure-lifecycle-group"})
		gomega.Expect(deleteMeasureErr).ShouldNot(gomega.HaveOccurred())
		_, deleteGroupErr := registry.DeleteGroup(ctx, "measure-lifecycle-group")
		gomega.Expect(deleteGroupErr).ShouldNot(gomega.HaveOccurred())
	})
})
...
var _ = ginkgo.Describe("Mixed Schema Cluster Operations", func() {
	...
	createMultiNodeRegistry := func(addresses ...string) *property.SchemaRegistry {
		multiReg := property.NewSchemaRegistryClient(newMockDialOptionsProvider(), 5*time.Second, 100*time.Millisecond)
	...
	ginkgo.It("should handle mixed schema deletions across cluster and verify via polling", func() {
		...
		observerReg := property.NewSchemaRegistryClient(newMockDialOptionsProvider(), 5*time.Second, 100*time.Millisecond)
		defer func() {
