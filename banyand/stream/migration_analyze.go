...
// StreamAnalyzeDiffResult is the source-vs-target multiset diff for stream.
...
type StreamAnalyzeDiffResult struct {
	Missing     []StreamAnalyzeMissingRow
	Extra       []StreamAnalyzeExtraRow
	SourceRows  uint64
	TargetRows  uint64
	MissingRows uint64
	MissingKeys int
...
// StreamAnalyzeMissingRow names one (seriesID, ts, elementID) tuple that
// exists in source but not in target after a migration copy.
type StreamAnalyzeMissingRow struct {
	PartPath  string
...
// StreamAnalyzeGroupRows walks every source part under srcRoots, decodes only
// block metadata + timestamps + elementIDs (NOT tag bodies — cheap scan), and
// returns per-row diagnostics. For stream, TotalRows == UniqueKeys always (no
// dedup); any MissingRows in a src-vs-tgt diff is a real bug.
type StreamAnalyzeGroupResult struct {
	PartsScanned uint64
	TotalRows    uint64
	UniqueKeys   uint64
}
...
// StreamAnalyzeGroupRows scans all source parts and counts rows.
func StreamAnalyzeGroupRows(srcRoots []string, fileSystem fs.FileSystem) (StreamAnalyzeGroupResult, error) {
	var res StreamAnalyzeGroupResult
	...
	err := walkStreamGroupParts(srcRoots, func(shardDir, _ string, partID uint64) error {
		rows, scanErr := scanOneStreamPartRows(
			partID, shardDir, fileSystem, &decoder,
			&compressed, &raw, &bms,
		)
		if scanErr != nil {
			return scanErr
		...
		atomic.AddUint64(&res.UniqueKeys, rows) // stream: every row is unique
...
func scanOneStreamPartRows(
	...
	_ = decoder // kept for future timestamp-level analysis
	return partRows, nil
}

// StreamAnalyzeGroupDiffWithTarget builds a (seriesID, ts, elementID) multiset
...
func StreamAnalyzeGroupDiffWithTarget(
	srcRoots []string,
	targetGroupRoot string,
	fileSystem fs.FileSystem,
	sampleCap int,
) (StreamAnalyzeDiffResult, error) {
	var res StreamAnalyzeDiffResult
	fileSystem = nofsyncStreamFS{FileSystem: fileSystem}
	...
	for k, srcPaths := range srcKeys {
		...
		if !inTgt || len(tgtPaths) == 0 {
			res.MissingRows += uint64(len(srcPaths))
			res.MissingKeys++
			if sampleCap == 0 || len(res.Missing) < sampleCap {
				res.Missing = append(res.Missing, StreamAnalyzeMissingRow{
					SeriesID:  k.sid,
					Timestamp: k.ts,
		...
		} else if len(tgtPaths) > len(srcPaths) {
			extra := uint64(len(tgtPaths) - len(srcPaths))
			res.ExtraRows += extra
			res.ExtraKeys++
			if sampleCap == 0 || len(res.Extra) < sampleCap {
				res.Extra = append(res.Extra, StreamAnalyzeExtraRow{
					SeriesID:  k.sid,
					Timestamp: k.ts,
					ElementID: k.elementID,
					PartPath:  tgtPaths[0],
...
func buildStreamKeyMultiset(
	...
	err := walkStreamGroupParts(roots, func(shardDir, partPath string, partID uint64) error {
		return scanOneStreamPartIntoMap(
			...
			keys, &decoder, &compressed, &raw, &bms,
		)
	})
	if err != nil {
		return nil, err
...
func scanOneStreamPartIntoMap(
	...
	_ = decoder // retained for future tag-level analysis
	return nil
}

// StreamAnalyzeMissingRowsByPartPath returns a sorted slice of unique part
// paths from a diff result, useful for operator diagnostics.
func StreamAnalyzeMissingRowsByPartPath(diff StreamAnalyzeDiffResult) []string {
	seen := make(map[string]struct{}, len(diff.Missing))
	for _, m := range diff.Missing {
		seen[m.PartPath] = struct{}{}
	}
