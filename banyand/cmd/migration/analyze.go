...
target part).`,
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			if kind == catalogStream {
				return runStreamAnalyze(plan, entryIdx, groupName, sampleCap)
			}
			return runMeasureAnalyze(plan, entryIdx, groupName, sampleCap)
		},
	}
...
func runMeasureAnalyze(plan *CopyPlan, entryIdx int, groupName string, sampleCap int) error {
...
func runStreamAnalyze(plan *CopyPlan, entryIdx int, groupName string, sampleCap int) error {
	...
	srcRoots := stream.StreamResolveEntrySrcRoots(cfg, entry, groupName)
	if len(srcRoots) == 0 {
		return fmt.Errorf("no source dirs resolve for entry-idx=%d group=%s (PVC likely doesn't carry this group)", entryIdx, groupName)
	}
	...
	start := time.Now()
	res, err := stream.StreamAnalyzeGroupRows(srcRoots, fs.NewLocalFileSystem())
	if err != nil {
		return err
	}
	elapsed := time.Since(start)
	...
	fmt.Printf("  tgt: %s\n", targetGroupRoot)
	diffStart := time.Now()
	diff, derr := stream.StreamAnalyzeGroupDiffWithTarget(srcRoots, targetGroupRoot, fs.NewLocalFileSystem(), sampleCap)
	if derr != nil {
		return derr
	}
	fmt.Printf("  src rows           : %d\n", diff.SourceRows)
