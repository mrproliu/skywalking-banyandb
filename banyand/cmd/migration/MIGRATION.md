...
Operator-facing procedure to re-normalize **measure** data on a BanyanDB deployment when the on-disk state has drifted from what the runtime expects. Common triggers:
...
- A partial restore that landed measure data outside the active stage's `SegmentInterval` grid.
- General "clean the historical measure tree without losing rows" maintenance — re-bucket every source row onto a fresh, grid-aligned target tree that the BanyanDB runtime can pick up unchanged.

Two end-to-end procedures, depending on where the source data lives:

- [Part 1 — Local single-host flow](#part-1--local-single-host-flow): the source is a backup snapshot downloaded to a workstation, the target is a fresh measure-data root you ship into the cluster afterwards.
- [Part 2 — Kubernetes live in-place flow](#part-2--kubernetes-live-in-place-flow): the source is the live cluster's own data PVCs; a helper pod rewrites each PVC's measure tree in place, the operator swaps the migrated groups onto every PVC, then the data StatefulSets restart on top of the clean tree.
...
The plan loader will pick up `measure` from every node directory and `schema-property` from whichever hot / schema-server nodes carry it.

## 3. Write a migration plan

Copy [`example/plan-backup.yaml`](example/plan-backup.yaml) and adapt every path / date / group / target to your environment. The shipped sample uses `/tmp/...` paths so it works out of the box on a fresh workstation:
...
2026/05/20 11:57:55 [migration] group sw_metricsMinute: building union sidx from 5 source dir(s) merged across all entries
2026/05/20 11:58:44 [migration] group sw_metricsHour: building union sidx from 5 source dir(s) merged across all entries
2026/05/20 11:59:35 [migration] group sw_metricsDay: building union sidx from 5 source dir(s) merged across all entries
2026/05/20 12:00:49 [migration] entry [1/5] stage=hot nodes=[hot-0] target=/tmp/banyandb-copy/hot/measure-data
2026/05/20 12:00:49 [migration] entry [1/5] stage=hot group sw_metricsMinute: 1 source dir(s), 34 parts, interval=day×1 — writing target (target="/tmp/banyandb-copy/hot/measure-data/sw_metricsMinute", union sidx="…/staging/groups/sw_metricsMinute/sidx")
2026/05/20 12:00:49 [migration] entry [1/5] stage=hot group sw_metricsMinute: part 1/34 (2.9%) done seg-20260519/shard-0/0000000000002c4c rows=18907 targetParts=1
...
2026/05/20 12:00:50 [migration] entry [1/5] stage=hot group sw_metricsMinute: done rows=7706398 segs=2 srcParts=34 targetParts=34
```
...
Start a standalone BanyanDB process pointing at one entry's target:

```bash
./build/banyandb standalone \
    --measure-root=/tmp/banyandb-copy/hot/measure-data \
    --grpc-addr=127.0.0.1:17912 \
    --http-addr=127.0.0.1:17913
```
...
The script probes the most recent `DAYS` (default 15) UTC days through the liaison HTTP API — full day for `sw_metricsDay`, the `08:00..09:00` hour for `sw_metricsHour`, the `08:08..08:09` minute for `sw_metricsMinute`. Any zero response is tagged `★MISSING`. Override `DAYS` to widen / narrow the window, or `LIAISON_HTTP` to point at a different liaison endpoint.

## 6. Ship the copy into your cluster

The copy-mode output is a measure-data root, byte-compatible with what a running BanyanDB pod expects. Common ways to bring it into a cluster: package the directory into a volume or image and start a pod with `--measure-root` pointing at it; or stop the BanyanDB process on the target node, swap the measure-data directory, and restart. For an end-to-end in-place flow inside Kubernetes, see [Part 2](#part-2--kubernetes-live-in-place-flow).

## 7. Rollback

Copy mode never touches the source backup. To roll back, just delete the target measure-data roots produced in Step 4:

```bash
rm -rf /tmp/banyandb-copy/{hot,warm,cold}/measure-data
```
...
End-to-end procedure to migrate measure data **in place** inside a live BanyanDB cluster running on Kubernetes. The runner pod mounts every data PVC, runs the migration into a `data-copy/` sibling on the same volume, and the operator then swaps `data/` ↔ `data-copy/` per PVC before resuming traffic.
...
Copy [`example/runner-pod.yaml`](example/runner-pod.yaml) and adapt it to your real cluster: PVC `claimName`s, mountPaths, resource limits, and the container `image` (use the same banyandb image your cluster runs — it already ships `banyand-migration` at `/banyand-migration`). The shipped sample mounts each measure PVC at `/mnt/<node>/measure` and each hot schema-property PVC at `/mnt/<node>/schema-property`, plus an `emptyDir` at `/work` for the plan file and per-group union sidx staging.
...
The shipped sample uses the **live** source mode — every entry sources from its own PVC's `/mnt/<node>/measure/data` and targets `data-copy` on the same volume:
...
source:
  live:
    ...
    stages:
      ...
      warm:
        - { node: warn-0, root: /mnt/warn-0/measure/data }
        - { node: warn-1, root: /mnt/warn-1/measure/data }
      cold:
        - { node: cold-0, root: /mnt/cold-0/measure/data }

groups: [sw_metricsMinute, sw_metricsHour, sw_metricsDay]

entries:
  - { stage: hot,  target: /mnt/hot-0/measure/data-copy,  nodes: [hot-0]  }
  - { stage: hot,  target: /mnt/hot-1/measure/data-copy,  nodes: [hot-1]  }
  - { stage: warm, target: /mnt/warn-0/measure/data-copy, nodes: [warn-0] }
  - { stage: warm, target: /mnt/warn-1/measure/data-copy, nodes: [warn-1] }
  - { stage: cold, target: /mnt/cold-0/measure/data-copy, nodes: [cold-0] }
```
...
A missing source/copy dir on some PVC is normal — banyandb's hash sharding means each replica only carries a subset of groups.
...
For each of the most recent `DAYS` (default 15) UTC days, the script issues a `stages=[hot,warm,cold]` query against each tier at the granularity that tier stores:

| Tier | Measure | Window |
|---|---|---|
| Day    | `service_apdex_day`    | full day (`00:00:00 .. 23:59:59`) |
| Hour   | `service_apdex_hour`   | the `08:00` hour (`08:00:00 .. 08:59:59`) |
| Minute | `service_apdex_minute` | the `08:08` minute (`08:08:00 .. 08:08:59`) |

A trailing `★MISSING` marker is appended whenever the probe returns zero data points for that day. Investigate any `★MISSING` row before reattaching client traffic.
